...
## 项目四: LSH-BC (轻谱混合自适应波段选择)

LSH-BC - Hyperspectral Image Classification Method Based on Light Spectrum Hybrid Adaptive Band Selection - 基于轻谱混合自适应波段选择的高光谱图像分类方法
该项目实现了一种端到端的“边分类边选择”的高光谱图像分类方法。它没有采用强化学习，而是设计了一个新颖的轻谱混合网络，通过内置的交叉注意力机制和联合损失函数，在训练分类器的同时，自适应地学习出每个光谱波段的重要性权重。

### 核心创新点

* **前端可学习波段加权 (`2_lsh_model.py`)**: 模型在数据输入的第一层就为所有光谱波段建立了一个可学习的权重向量。该向量直接参与网络的前向和反向传播，使得波段权重可以与分类任务一同进行端到端的优化，克服了传统方法中波段选择与分类任务脱节的问题。
* **轻谱混合网络架构 (`2_lsh_model.py`)**: 采用“局部卷积+轻量全局变换器”的高效混合结构。

  * 首先用一个3D卷积提取局部的空谱特征并进行通道降维。
  * 然后将特征图块化，送入轻量Transformer结构中捕捉长距离依赖关系。
  * 最后将局部与全局特征融合，在降低模型参数和计算量的同时，高效提取了多尺度的判别信息。
* **交叉注意引导的权重动态更新 (`2_lsh_model.py` & `4_trainer.py`)**: 这是实现“自适应”选择的核心。它利用网络高层特征作为查询(Query)，浅层特征作为键(Key)，动态计算出各波段对当前分类任务的贡献度。这个贡献度随后通过动量更新的方式，实时地“写回”到前端的可学习权重向量中，形成了一个“分类反馈→注意力评估→权重回写”的闭环。
* **联合优化损失 (`3_loss_function.py`)**: 损失函数由两部分构成：标准的交叉熵分类损失和波段正则损失。

  * 分类损失确保模型的分类精度。
  * 波段正则损失则包含一个**稀疏项**（鼓励模型使用更少的波段）和一个**熵项**（鼓励权重决策更明确，避免模糊），使得模型在“高精度”和“高稀疏性”之间自动寻找最优平衡。

### 项目结构 (`LSH-BC/`)

```
LSH-BC/
├── 1_data_loader.py          # 负责数据加载、立方体构建和分层采样
├── 2_lsh_model.py            # 定义核心的LSH-BC网络模型
├── 3_loss_function.py        # 定义联合优化损失函数
├── 4_trainer.py              # 负责实现完整的训练、验证和权重更新逻辑
├── main.py                   # 主程序入口，配置和启动整个流程
└── requirements.txt          # 项目依赖
...
```
所有依赖项都列在 `LSH-BC/requirements.txt` 文件中。
torch
scikit-learn
numpy
scipy
...
1. [cite_start]**准备数据**: 专利中使用了萨利纳斯(Salinas)数据集 [cite: 11]。运行前，请确保 `Salinas_corrected.mat` 和 `Salinas_gt.mat` 数据文件已下载，并相应修改 `main.py` 文件顶部的绝对路径配置。
2. **启动完整流程**: 直接运行 `main.py` 文件。

   ```bash
   python LSH-BC/main.py
   ```
   程序将依次执行以下步骤：

   1. [cite_start]加载数据，构建数据立方体并进行分层抽样 [cite: 11]。
   2. [cite_start]初始化轻谱混合网络模型、联合损失函数及优化器 [cite: 11]。
   3. [cite_start]开始训练，在每个批次中同时更新网络参数和自适应的波段权重 [cite: 11]。
   4. 训练结束后，报告在测试集上的最佳分类精度，并打印出模型学习到的最终波段权重分布。
